/**
 * QR Links AR Video Experience Page
 * Dedicated full-screen AR experience for QR Links AR Video campaigns
 * Displays video, documents, contact information, and social links upon target detection
 */

import React from 'react';
import { useParams } from 'react-router-dom';

// Hooks
import { useARState } from '../../hooks/useARState';
import { useDebug } from '../../hooks/useDebug';
import { useAnalytics } from '../../hooks/useAnalytics';
import { useProjectData } from '../../hooks/useProjectData';
import { useARLogic } from '../../hooks/useARLogic';

// Utils
import { checkLibraries } from '../../utils/arUtils';
import { shouldTrackAnalytics, markAnalyticsFailed } from '../../utils/analyticsDeduplication';
import { analyticsAPI } from '../../utils/api';

// Components
import DebugPanel from '../../components/AR/DebugPanel';
import LoadingScreen from '../../components/AR/LoadingScreen';
import ErrorScreen from '../../components/AR/ErrorScreen';
import ARControls from '../../components/AR/ARControls';
import ProjectDisabledScreen from '../../components/AR/ProjectDisabledScreen';
import CompositeImageOverlay from '../../components/AR/CompositeImageOverlay';
import ScannerAnimation from '../../components/AR/ScannerAnimation';
import DirectVideoPlayer from '../../components/AR/DirectVideoPlayer';
import ThemeRenderer from '../../components/Templates/ThemeRenderer';

const QRLinksARVideoExperiencePage = () => {
  const { userId, projectId } = useParams();

  // State management
  const arState = useARState();
  const {
    isInitialized,
    isScanning,
    projectData,
    error,
    isLoading,
    librariesLoaded,
    cameraActive,
    arReady,
    targetDetected,
    videoPlaying,
    videoMuted,
    showDebug,
    debugMessages,
    setIsLoading,
    setProjectData,
    setError,
    setLibrariesLoaded,
    setCameraActive,
    setArReady,
    setTargetDetected,
    setVideoPlaying,
    setVideoMuted,
    setIsInitialized,
    setIsScanning,
    setShowDebug,
    setDebugMessages,
    resetARState,
    cameraPermissionRequired,
    cameraPermissionBlocked,
    cameraPermissionDismissed,
    setCameraPermissionRequired,
    setCameraPermissionBlocked,
    setCameraPermissionDismissed
  } = arState;

  // Check if project is disabled
  const [isProjectDisabled, setIsProjectDisabled] = React.useState(false);
  const [disabledProjectName, setDisabledProjectName] = React.useState('');
  
  // Composite image and scanner animation state
  const [showCompositeImage, setShowCompositeImage] = React.useState(false);
  const [showScannerAnimation, setShowScannerAnimation] = React.useState(false);
  const [hasShownInitialGuide, setHasShownInitialGuide] = React.useState(false);
  
  // Video controls overlay state
  const [showVideoControls, setShowVideoControls] = React.useState(true);
  const permissionPromptedRef = React.useRef(false);

  const attemptCameraPermission = React.useCallback(async () => {
    if (!navigator?.mediaDevices?.getUserMedia) {
      return false;
    }
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        video: { facingMode: 'environment' },
        audio: false
      });
      stream.getTracks().forEach((track) => track.stop());
      setCameraPermissionRequired(false);
      setCameraPermissionBlocked(false);
      setCameraPermissionDismissed(false);
      return true;
    } catch (err) {
      const message = (err?.message || '').toLowerCase();
      const dismissed = message.includes('dismissed');
      setCameraPermissionRequired(true);
      setCameraPermissionBlocked(!dismissed);
      setCameraPermissionDismissed(dismissed);
      return false;
    }
  }, [setCameraPermissionRequired, setCameraPermissionBlocked, setCameraPermissionDismissed]);

  // Debug utilities
  const { addDebugMessage } = useDebug(setDebugMessages);

  // Get effective userId from URL params or from projectData after it loads
  const effectiveUserId = React.useMemo(() => {
    return userId || projectData?.userId;
  }, [userId, projectData?.userId]);

  // Preload composite image when project data is loaded
  React.useEffect(() => {
    if (projectData?.compositeDesignUrl || projectData?.designUrl) {
      const img = new Image();
      img.src = projectData.compositeDesignUrl || projectData.designUrl;
      addDebugMessage('📷 Preloading composite image...', 'info');
    }
  }, [projectData?.compositeDesignUrl, projectData?.designUrl, addDebugMessage]);

  // Analytics
  const { trackAnalytics } = useAnalytics(effectiveUserId, projectId, addDebugMessage);

  // Project data
  const { fetchProjectData } = useProjectData(
    projectId, 
    userId, 
    setIsLoading, 
    setProjectData, 
    setError, 
    addDebugMessage, 
    trackAnalytics,
    setIsProjectDisabled,
    setDisabledProjectName
  );

  // AR logic
  const arLogic = useARLogic({
    librariesLoaded,
    projectData,
    userId: effectiveUserId,
    projectId,
    isInitialized,
    isScanning,
    targetDetected,
    videoPlaying,
    videoMuted,
    setTargetDetected,
    setVideoPlaying,
    setVideoMuted,
    setArReady,
    setCameraActive,
    setIsInitialized,
    setIsScanning,
    addDebugMessage,
    trackAnalytics
  });

  // Check libraries on mount
  React.useEffect(() => {
    checkLibraries(setLibrariesLoaded, addDebugMessage);
  }, [setLibrariesLoaded, addDebugMessage]);

  // Fetch project data on mount
  React.useEffect(() => {
    console.log('📦 Fetching project data (once)...');
    fetchProjectData();
  }, [fetchProjectData]);

  // Auto-play on first detection
  const prevDetectedRef = React.useRef(false);
  React.useEffect(() => {
    if (targetDetected && !prevDetectedRef.current) {
      setVideoPlaying(true);
      setShowCompositeImage(false);
      setShowScannerAnimation(false);
    }
    prevDetectedRef.current = targetDetected;
  }, [targetDetected, setVideoPlaying]);

  // Show composite image immediately when project data is loaded
  React.useEffect(() => {
    if (projectData && !targetDetected && !hasShownInitialGuide && !isLoading) {
      if (projectData.requiresTargetImage === false) {
        setHasShownInitialGuide(true);
        addDebugMessage('📹 Direct video mode - no target guide needed', 'info');
      } else if (projectData?.compositeDesignUrl || projectData?.designUrl) {
        setShowCompositeImage(true);
        setShowScannerAnimation(true);
        setHasShownInitialGuide(true);
        addDebugMessage('📱 Showing composite image guide to user', 'info');
      }
    }
  }, [projectData, targetDetected, hasShownInitialGuide, isLoading, projectData?.compositeDesignUrl, projectData?.designUrl, addDebugMessage]);

  // Extract data from projectData
  const phygitalizedData = projectData?.phygitalizedData || {};
  
  // Get documentUrls from multiple sources
  const documentUrls = React.useMemo(() => {
    const phygitalizedDocs = phygitalizedData?.documentUrls || [];
    const uploadedDocs = projectData?.uploadedFiles?.documents || [];
    const legacyDocs = projectData?.documentUrls || [];
    
    const uploadedDocUrls = uploadedDocs.map(doc => 
      typeof doc === 'string' ? doc : doc.url
    ).filter(Boolean);
    
    const allDocs = [...phygitalizedDocs, ...uploadedDocUrls, ...legacyDocs];
    return [...new Set(allDocs)];
  }, [phygitalizedData?.documentUrls, projectData?.uploadedFiles?.documents, projectData?.documentUrls]);

  const phoneNumber = phygitalizedData?.phoneNumber || null;
  const whatsappNumber = phygitalizedData?.whatsappNumber || null;
  const socialLinks = phygitalizedData?.socialLinks || projectData?.socialLinks || {};
  
  // Get template data from phygitalizedData or projectData (backward compatibility)
  const templateId = phygitalizedData?.templateId || projectData?.templateId || 'default';
  const templateConfig = phygitalizedData?.templateConfig || projectData?.templateConfig || {};
  
  // Debug logging
  React.useEffect(() => {
    console.log('🎨 AR Experience Template Debug:', {
      templateId,
      templateConfig,
      hasPhygitalizedData: !!phygitalizedData,
      phygitalizedDataKeys: phygitalizedData ? Object.keys(phygitalizedData) : [],
      phygitalizedDataTemplateId: phygitalizedData?.templateId,
      phygitalizedDataTemplateConfig: phygitalizedData?.templateConfig,
      projectDataKeys: projectData ? Object.keys(projectData) : [],
      projectDataTemplateId: projectData?.templateId,
      projectDataTemplateConfig: projectData?.templateConfig,
      fullProjectData: projectData,
      fullPhygitalizedData: phygitalizedData
    });
  }, [templateId, templateConfig, phygitalizedData, projectData]);

  const sanitizeNumber = (num) => (num || '').replace(/[^0-9+]/g, '');
  
  const handleSocialClick = (platform, url) => {
    try {
      if (url) {
        const additionalData = { platform };
        
        if (shouldTrackAnalytics('linkClick', effectiveUserId, projectId, additionalData)) {
          const eventId = additionalData.eventId;
          
          console.log('🔗 Social link clicked:', { platform, url, userId: effectiveUserId, projectId, eventId });
          
          analyticsAPI.trackLinkClick(effectiveUserId, platform, url, projectId, eventId).then(() => {
            console.log('✅ Link click tracked successfully:', { platform, eventId });
          }).catch((err) => {
            console.warn('⚠️ Link click tracking failed:', err);
            markAnalyticsFailed('linkClick', effectiveUserId, projectId, { platform });
          });
        }
      }
    } catch (err) {
      console.error('❌ Error in handleSocialClick:', err);
    }
  };

  // Render loading screen
  if (isLoading || !librariesLoaded) {
    return <LoadingScreen />;
  }

  // Render error screen
  if (error) {
    return <ErrorScreen error={error} onRetry={() => fetchProjectData()} />;
  }

  // Render disabled project screen
  if (isProjectDisabled) {
    return <ProjectDisabledScreen projectName={disabledProjectName} />;
  }

  // Get template background style for direct application
  const getTemplateBackground = React.useMemo(() => {
    // Priority 1: Custom background color from templateConfig
    if (templateConfig?.customBackgroundColor) {
      return templateConfig.customBackgroundColor
    }
    
    if (templateId === 'default') {
      return 'linear-gradient(to bottom right, #0F172A, #1E293B, #0F172A)'
    }
    
    // Import template data
    const { getTemplate } = require('../../config/templates')
    const template = getTemplate(templateId)
    
    // Priority 2: Prefer colors.background
    if (template?.colors?.background) {
      return template.colors.background
    }
    
    // Priority 3: Fallback to converting Tailwind classes
    if (template?.components?.background?.value) {
      const bgClasses = template.components.background.value
      if (bgClasses.includes('bg-gradient-to-b') && bgClasses.includes('from-blue-900')) {
        return 'linear-gradient(to bottom, #1E3A8A, #1E40AF, #1E3A8A)'
      } else if (bgClasses.includes('bg-gradient-to-br') && bgClasses.includes('from-purple-900')) {
        return 'linear-gradient(to bottom right, #4C1D95, #5B21B6, #6D28D9)'
      } else if (bgClasses.includes('from-pink-500')) {
        return 'linear-gradient(135deg, #EC4899, #3B82F6, #10B981, #FCD34D, #A855F7)'
      }
    }
    
    // Default fallback
    return 'linear-gradient(to bottom right, #0F172A, #1E293B, #0F172A)'
  }, [templateId, templateConfig])

  return (
    <ThemeRenderer template={templateId} templateConfig={templateConfig}>
      <div 
        className="min-h-screen w-screen h-screen fixed inset-0 overflow-hidden" 
        style={{ 
          background: getTemplateBackground || 'transparent',
          ...(getTemplateBackground && { backgroundColor: 'transparent' })
        }}
      >
      {/* AR Container - Full Screen */}
      <div 
        id="ar-container" 
        className="absolute inset-0 w-full h-full"
        style={{ zIndex: 1 }}
      />

      {/* Content Overlay */}
      {targetDetected && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm overflow-y-auto">
          {/* Video Player - Full Screen */}
          {projectData?.videoUrl && (
            <div className="w-full max-w-4xl mx-auto mb-6">
              <DirectVideoPlayer
                videoUrl={projectData.videoUrl}
                playing={videoPlaying}
                muted={videoMuted}
                onPlay={() => setVideoPlaying(true)}
                onPause={() => setVideoPlaying(false)}
                onMute={() => setVideoMuted(true)}
                onUnmute={() => setVideoMuted(false)}
              />
            </div>
          )}

          {/* Documents Section */}
          {documentUrls && documentUrls.length > 0 && (
            <div className="mt-6 md:mt-8 w-full max-w-4xl mx-auto px-4">
              <h2 className="text-base md:text-lg font-bold text-slate-100 mb-4 md:mb-5 text-center">Documents</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                {documentUrls.map((docUrl, index) => {
                  const isPdf = docUrl.toLowerCase().includes('.pdf') || docUrl.toLowerCase().includes('pdf');
                  const isImage = docUrl.toLowerCase().match(/\.(jpg|jpeg|png|gif|webp)$/);
                  const fileName = docUrl.split('/').pop() || `Document ${index + 1}`;
                  
                  return (
                    <a
                      key={index}
                      href={docUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="border border-slate-600/30 rounded-lg md:rounded-xl p-4 md:p-5 flex flex-col items-center justify-center space-y-2 md:space-y-3 transition-all duration-200 touch-manipulation backdrop-blur-md group"
                      style={{ backgroundColor: 'var(--theme-card, rgba(30, 41, 59, 0.8))' }}
                    >
                      <div className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center">
                        {isPdf ? (
                          <svg viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 md:w-10 md:h-10 group-hover:scale-110 transition-transform" style={{ color: 'var(--theme-primary, #EF4444)' }}>
                            <path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2M18,20H6V4H13V9H18V20Z" />
                          </svg>
                        ) : isImage ? (
                          <svg viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 md:w-10 md:h-10 text-blue-500 group-hover:scale-110 transition-transform">
                            <path d="M21,19V5C21,3.89 20.1,3 19,3H5A2,2 0 0,0 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19M19,19H5V5H19V19M13.96,12.29L11.21,15.83L9.25,13.47L6.5,17H17.5L13.96,12.29Z" />
                          </svg>
                        ) : (
                          <svg viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8 md:w-10 md:h-10 text-slate-400 group-hover:scale-110 transition-transform">
                            <path d="M14,2H6A2,2 0 0,0 4,4V20A2,2 0 0,0 6,22H18A2,2 0 0,0 20,20V8L14,2M18,20H6V4H13V9H18V20Z" />
                          </svg>
                        )}
                      </div>
                      <span className="text-xs md:text-sm font-medium text-slate-100 text-center truncate w-full px-2">
                        {fileName.length > 20 ? `${fileName.substring(0, 20)}...` : fileName}
                      </span>
                    </a>
                  );
                })}
              </div>
            </div>
          )}

          {/* Contact Information */}
          {(phoneNumber || whatsappNumber) && (
            <div className="mt-6 md:mt-8 w-full max-w-2xl mx-auto px-4">
              <h2 className="text-base md:text-lg font-bold text-slate-100 mb-4 md:mb-5 text-center">Contact Information</h2>
              <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center">
                {phoneNumber && (
                  <a
                    href={`tel:${sanitizeNumber(phoneNumber)}`}
                    className="border border-slate-600/30 rounded-lg md:rounded-xl p-4 md:p-5 flex items-center justify-center space-x-3 transition-all duration-200 touch-manipulation backdrop-blur-md"
                    style={{ backgroundColor: 'var(--theme-card, rgba(30, 41, 59, 0.8))' }}
                  >
                    <svg className="w-6 h-6 md:w-8 md:h-8" style={{ color: 'var(--theme-primary, #00D4FF)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                    </svg>
                    <span className="text-sm md:text-base font-medium text-slate-100">{phoneNumber}</span>
                  </a>
                )}
                {whatsappNumber && (
                  <a
                    href={`https://wa.me/${sanitizeNumber(whatsappNumber)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="border border-slate-600/30 rounded-lg md:rounded-xl p-4 md:p-5 flex items-center justify-center space-x-3 transition-all duration-200 touch-manipulation backdrop-blur-md"
                    style={{ backgroundColor: 'var(--theme-card, rgba(30, 41, 59, 0.8))' }}
                  >
                    <svg className="w-6 h-6 md:w-8 md:h-8" style={{ color: 'var(--theme-secondary, #10B981)' }} fill="currentColor" viewBox="0 0 24 24">
                      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                    </svg>
                    <span className="text-sm md:text-base font-medium text-slate-100">{whatsappNumber}</span>
                  </a>
                )}
              </div>
            </div>
          )}

          {/* Social Links */}
          {socialLinks && Object.keys(socialLinks).length > 0 && (
            <div className="mt-6 md:mt-8 w-full max-w-2xl mx-auto px-4">
              <h2 className="text-base md:text-lg font-bold text-slate-100 mb-4 md:mb-5 text-center">Follow Us</h2>
              <div className="flex flex-wrap gap-3 md:gap-4 justify-center">
                {socialLinks.website && (
                  <button
                    onClick={() => handleSocialClick('website', socialLinks.website)}
                    className="border border-slate-600/30 rounded-lg md:rounded-xl p-4 md:p-5 transition-all duration-200 touch-manipulation backdrop-blur-md"
                    style={{ backgroundColor: 'var(--theme-card, rgba(30, 41, 59, 0.8))' }}
                  >
                    <svg className="w-6 h-6 md:w-8 md:h-8" style={{ color: 'var(--theme-primary, #00D4FF)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                    </svg>
                  </button>
                )}
                {socialLinks.instagram && (
                  <button
                    onClick={() => handleSocialClick('instagram', socialLinks.instagram)}
                    className="border border-slate-600/30 rounded-lg md:rounded-xl p-4 md:p-5 transition-all duration-200 touch-manipulation backdrop-blur-md"
                    style={{ backgroundColor: 'var(--theme-card, rgba(30, 41, 59, 0.8))' }}
                  >
                    <svg className="w-6 h-6 md:w-8 md:h-8" style={{ color: 'var(--theme-primary, #EC4899)' }} fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                    </svg>
                  </button>
                )}
                {socialLinks.facebook && (
                  <button
                    onClick={() => handleSocialClick('facebook', socialLinks.facebook)}
                    className="border border-slate-600/30 rounded-lg md:rounded-xl p-4 md:p-5 transition-all duration-200 touch-manipulation backdrop-blur-md"
                    style={{ backgroundColor: 'var(--theme-card, rgba(30, 41, 59, 0.8))' }}
                  >
                    <svg className="w-6 h-6 md:w-8 md:h-8" style={{ color: 'var(--theme-primary, #3B82F6)' }} fill="currentColor" viewBox="0 0 24 24">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                  </button>
                )}
                {socialLinks.twitter && (
                  <button
                    onClick={() => handleSocialClick('twitter', socialLinks.twitter)}
                    className="border border-slate-600/30 rounded-lg md:rounded-xl p-4 md:p-5 transition-all duration-200 touch-manipulation backdrop-blur-md"
                    style={{ backgroundColor: 'var(--theme-card, rgba(30, 41, 59, 0.8))' }}
                  >
                    <svg className="w-6 h-6 md:w-8 md:h-8" style={{ color: 'var(--theme-primary, #60A5FA)' }} fill="currentColor" viewBox="0 0 24 24">
                      <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                    </svg>
                  </button>
                )}
                {socialLinks.linkedin && (
                  <button
                    onClick={() => handleSocialClick('linkedin', socialLinks.linkedin)}
                    className="border border-slate-600/30 rounded-lg md:rounded-xl p-4 md:p-5 transition-all duration-200 touch-manipulation backdrop-blur-md"
                    style={{ backgroundColor: 'var(--theme-card, rgba(30, 41, 59, 0.8))' }}
                  >
                    <svg className="w-6 h-6 md:w-8 md:h-8" style={{ color: 'var(--theme-primary, #2563EB)' }} fill="currentColor" viewBox="0 0 24 24">
                      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                    </svg>
                  </button>
                )}
              </div>
            </div>
          )}
        </div>
      )}

      {/* Composite Image Overlay - Show when target not detected */}
      {!targetDetected && showCompositeImage && projectData?.compositeDesignUrl && (
        <CompositeImageOverlay 
          imageUrl={projectData.compositeDesignUrl} 
          onClose={() => setShowCompositeImage(false)}
        />
      )}

      {/* Scanner Animation - Show when target not detected */}
      {!targetDetected && showScannerAnimation && (
        <ScannerAnimation />
      )}

      {/* AR Controls */}
      {targetDetected && (
        <ARControls
          videoPlaying={videoPlaying}
          videoMuted={videoMuted}
          onToggleVideo={() => setVideoPlaying(!videoPlaying)}
          onToggleMute={() => setVideoMuted(!videoMuted)}
          onRestart={() => {
            resetARState();
            setTargetDetected(false);
            setVideoPlaying(false);
            setShowCompositeImage(true);
            setShowScannerAnimation(true);
          }}
        />
      )}

      {/* Debug Panel */}
      {showDebug && (
        <DebugPanel 
          messages={debugMessages} 
          onClose={() => setShowDebug(false)}
        />
      )}
      </div>
    </ThemeRenderer>
  );
};

export default QRLinksARVideoExperiencePage;
