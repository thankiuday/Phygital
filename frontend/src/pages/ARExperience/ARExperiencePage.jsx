...
const ARExperiencePage = () => {
  ...
  const addDebugMessage = (message, type = 'info') => {
    ...
    if (isMobile) {
      ...
      // Auto-hide after 5 seconds for info messages, keep visible for errors
      ...
      <main className="flex-1 relative">
        {!isScanning ? (
          ...
          <>
            ...
            <div className="absolute top-4 left-4 z-20 bg-black bg-opacity-90 text-white px-3 py-2 rounded-lg max-w-xs">
              {cameraActive ? (
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-sm font-medium">
                      {scanningStatus === 'detected' ? '🎯 Target Found!' :
                       scanningStatus === 'scanning' ? '📷 Scanning...' :
                       scanningStatus === 'lost' ? '👁️ Target Lost' :
                       'Camera Active'}
                    </div>
                    {fallbackMode ? (
                      ...
                      <div className="text-xs text-green-300">
                        {scanningStatus === 'detected' ? 'Video should be playing' :
                         scanningStatus === 'scanning' ? 'Point camera at your design' :
                         scanningStatus === 'lost' ? 'Find your design again' :
                         'AR Mode - Point at design'}
                      </div>
                    )}
                  </div>
                </div>
            ...
            {/* Mobile Debug Panel - More Prominent */}
            {debugMessages.length > 0 && (
              <div className="absolute top-20 left-4 right-4 z-30">
                <button
                  onClick={() => setShowDebug(!showDebug)}
                  className={`w-full bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg text-sm mb-2 font-medium ${
                    showDebug ? 'bg-blue-700' : ''
                  }`}
                >
                  📱 Debug Info {showDebug ? '▼' : '▶'} ({debugMessages.length})
                </button>
                
                {showDebug && (
                  <div className="bg-black bg-opacity-95 text-white p-4 rounded-lg max-h-80 overflow-y-auto border border-blue-500">
                    ...
                    <div className="space-y-2">
                      {debugMessages.map((msg) => (
                        <div key={msg.id} className="text-sm border-l-2 pl-2 border-gray-600">
                          <div className="text-gray-400 text-xs">{msg.timestamp}</div>
                          <div className={`font-medium ${
                            msg.type === 'error' ? 'text-red-400 border-l-red-500' :
                            msg.type === 'warning' ? 'text-yellow-400 border-l-yellow-500' :
                            msg.type === 'success' ? 'text-green-400 border-l-green-500' :
