...
export default function WaveLayout({ card, colors, template, contactItems, trackEvent, downloadVCard }) {
  ...
  const visibleSections = (card.sections || []).filter(s => s.visible !== false).sort((a, b) => (a.order || 0) - (b.order || 0))
  ...
  const blocks = {
    ...
    photo: (blockId) => {
      ...
      const useOverlap = photoAfterBanner && showBanner && showPhoto
      return (
        <div key={blockId} className={`flex justify-center ${useOverlap ? '-mt-4 mb-2' : 'mt-4 mb-2'}`}>
          <img
            src={profile.photo}
    ...
    nameInfo: (blockId) => (
      <div key={blockId} className="text-center px-4 sm:px-6 -mt-2">
        {profile.name && (
    ...
    contact: (blockId) => {
      ...
      return (
        <div key={blockId} className="flex flex-wrap justify-center gap-3 sm:gap-4 px-4 sm:px-6 mt-5 sm:mt-6">
    ...
    saveContact: (blockId) => (
      <div key={blockId} className="flex justify-center px-4 sm:px-6 mt-4">
        <button
    ...
    sections: (blockId) => {
      ...
      return (
        <div key={blockId} className="px-4 sm:px-6 mt-4">
    ...
    social: (blockId) => (
      <div key={blockId} className="px-4 sm:px-6 pb-2">
