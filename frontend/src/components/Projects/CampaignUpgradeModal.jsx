...
const CampaignUpgradeModal = ({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="relative w-full max-w-2xl bg-slate-800 rounded-xl border border-slate-600/50 shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-slate-700/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-100">Upgrade Campaign Type</h2>
              <p className="text-sm text-slate-400 mt-1">Enhance your campaign with new features</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between p-4 bg-slate-900/50 rounded-lg border border-slate-700/30">
            <div className="flex items-center gap-3 flex-1">
              <div className={`w-12 h-12 bg-${currentInfo.color}-900/20 border border-${currentInfo.color}-600/50 rounded-lg flex items-center justify-center`}>
                <CurrentIcon className={`w-6 h-6 text-${currentInfo.color}-400`} />
              </div>
              <div>
                <p className="text-sm font-medium text-slate-400">Current Type</p>
                <p className="text-lg font-bold text-slate-100">{currentInfo.name}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 flex-1">
              <div className={`w-12 h-12 bg-${newInfo.color}-900/20 border border-${newInfo.color}-600/50 rounded-lg flex items-center justify-center`}>
                <NewIcon className={`w-6 h-6 text-${newInfo.color}-400`} />
              </div>
              <div>
                <p className="text-sm font-medium text-slate-400">New Type</p>
                <p className="text-lg font-bold text-slate-100">{newInfo.name}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {features.map((feature, index) => (
                <div key={index} className="flex items-center gap-3 p-3 bg-green-900/10 border border-green-600/20 rounded-lg">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-200 mb-3">Your Existing Data</h3>
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-slate-700/50 flex flex-col sm:flex-row justify-end gap-3">
          <button
            onClick={onClose}
            disabled={isUpgrading}
